...
public class DictionaryHashTable<K extends Comparable<? super K>, V> implements DictionaryInterface<K, V>
{
     ...
     public DictionaryHashTable(int size)
     {
         ...
         NodeD<K, V>[] tempDict = (NodeD<K, V>[])new NodeD<?,?>[size];
         dict = tempDict;
         for (int i=0; i<dict.length; i++)
         {
             dict[i] = new NodeD<K, V>(null, null); //fill dict with unused Nodes
         }
         numOfItems = 0;
     }
     ...
     {
         NodeD<K, V>[] tempDict = dict;
         @SuppressWarnings("unchecked")
         NodeD<K, V>[] newDict = (NodeD<K, V>[])new NodeD<?,?>[nextPrime()]; 	//make new array of next prime at least twice the size
         for (int i=0; i<newDict.length; i++)
         {
             newDict[i] = new NodeD<K, V>(null, null);		//populating new array with unused nodes
         }
         dict = newDict;
